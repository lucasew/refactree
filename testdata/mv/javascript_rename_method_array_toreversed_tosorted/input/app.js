...
function useToReversed() {
  return (
    [new A()].toReversed()[0].run() + [new B()].toReversed()[0].run()
  );
}
...
function useToReversedAt() {
  return (
    [new A()].toReversed().at(0).run() + [new B()].toReversed().at(0).run()
  );
}
...
function useToSorted() {
  return [new A()].toSorted()[0].run() + [new B()].toSorted()[0].run();
}

function useToSortedCmp() {
  return (
    [new A()].toSorted((x, y) => 0)[0].run() +
...
function useArrayFromToReversed() {
  return (
    Array.from([new A()]).toReversed()[0].run() +
...
function useIdent() {
  const a = new A();
  const b = new B();
  return [a].toReversed()[0].run() + [b].toSorted()[0].run();
}
