...
def use_dict_get_walrus(ba: BoxA, bb: BoxB) -> int:
    return (xa := {"k": ba.get()}.get("k")).run() + (xb := {"k": bb.get()}.get("k")).run()


def use_dict_getitem(ba: BoxA, bb: BoxB) -> int:
    return {"k": ba.get()}.__getitem__("k").run() + {"k": bb.get()}.__getitem__("k").run()


def use_list_repeat_inline(ba: BoxA, bb: BoxB) -> int:
    return ([ba.get()] * 2)[0].run() + ([bb.get()] * 2)[0].run()


def use_list_repeat_left(ba: BoxA, bb: BoxB) -> int:
    return (2 * [ba.get()])[0].run() + (2 * [bb.get()])[0].run()
...
def use_dict_kw_inline(ba: BoxA, bb: BoxB) -> int:
    return dict(k=ba.get())["k"].run() + dict(k=bb.get())["k"].run()


def use_dict_kw_get(ba: BoxA, bb: BoxB) -> int:
    return dict(k=ba.get()).get("k").run() + dict(k=bb.get()).get("k").run()
...
def use_dict_kw_multi(ba: BoxA, bb: BoxB) -> int:
    return dict(k=ba.get(), j=ba.get())["j"].run() + dict(k=bb.get(), j=bb.get())["j"].run()
