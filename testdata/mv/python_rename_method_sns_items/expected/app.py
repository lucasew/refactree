...
def use_items_next():
    return (
        next(iter(vars(SimpleNamespace(k=A())).items()))[1].execute()
...
def use_items_list():
    return (
        list(vars(SimpleNamespace(k=A())).items())[0][1].execute()
...
def use_items_types():
    return (
        next(iter(vars(types.SimpleNamespace(k=A())).items()))[1].execute()
...
def use_items_dunder():
    return (
        next(iter(SimpleNamespace(k=A()).__dict__.items()))[1].execute()
