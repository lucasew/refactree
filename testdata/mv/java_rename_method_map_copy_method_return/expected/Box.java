...
class Use {
  int useHashMap(BoxA ba, BoxB bb) {
    return new HashMap<>(Map.of("k", ba.get())).get("k").execute()
        + new HashMap<>(Map.of("k", bb.get())).get("k").run();
  }

  int useLinkedHashMap(BoxA ba, BoxB bb) {
    return new LinkedHashMap<>(Map.of("k", ba.get())).get("k").execute()
        + new LinkedHashMap<>(Map.of("k", bb.get())).get("k").run();
  }

  int useTreeMap(BoxA ba, BoxB bb) {
    return new TreeMap<>(Map.of("k", ba.get())).get("k").execute()
  ...
  // Class regression
  int useClass() {
    return new HashMap<>(Map.of("k", new A())).get("k").execute()
