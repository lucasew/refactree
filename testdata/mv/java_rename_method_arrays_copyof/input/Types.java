...
class Uses {
  public static int useCopyOfDirect(A[] as, B[] bs) {
    return Arrays.copyOf(as, as.length)[0].run() + Arrays.copyOf(bs, bs.length)[0].run();
  }

  public static int useCopyOfRangeDirect(A[] as, B[] bs) {
    return Arrays.copyOfRange(as, 0, 1)[0].run() + Arrays.copyOfRange(bs, 0, 1)[0].run();
  }
  ...
  public static int useCopyOfNew(A[] as, B[] bs) {
    return Arrays.copyOf(new A[] { as[0] }, 1)[0].run()
