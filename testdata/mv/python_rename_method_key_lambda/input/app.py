...
def use_sorted(items: list[A]):
    sorted(items, key=lambda x: x.run())
...
def use_min(items: list[A]):
    return min(items, key=lambda x: x.run())
...
def use_max(items: list[A]):
    return max(items, key=lambda x: x.run())
...
def use_map(items: list[A]):
    return list(map(lambda x: x.run(), items))
...
def use_filter(items: list[A]):
    return list(filter(lambda x: x.run(), items))
