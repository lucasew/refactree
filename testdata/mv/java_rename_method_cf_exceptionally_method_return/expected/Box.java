...
class Use {
  ...
  int useClassFailed() {
    return CompletableFuture.failedFuture(new RuntimeException()).exceptionally(t -> new A()).join().execute()
  ...
  int useMRFailed(BoxA ba, BoxB bb) {
    return CompletableFuture.failedFuture(new RuntimeException()).exceptionally(t -> ba.get()).join().execute()
