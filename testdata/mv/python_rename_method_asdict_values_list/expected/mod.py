...
def use_list(box: Box) -> int:
    return list(asdict(box).values())[0].execute()


def use_tuple(box: Box) -> int:
    return tuple(asdict(box).values())[0].execute()


def use_dc_list(box: Box) -> int:
    return list(dataclasses.asdict(box).values())[0].execute()


def use_vars_list(box: Box) -> int:
    return list(vars(box).values())[0].execute()


def use_dunder_list(box: Box) -> int:
    return list(box.__dict__.values())[0].execute()


def use_assigned(box: Box) -> int:
    d = asdict(box)
    return list(d.values())[0].execute()
