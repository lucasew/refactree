...
class Use {
  ...
  int useClassArrayList() {
    return new ArrayList<>(List.of(new A())).get(0).execute()
        + new ArrayList<>(List.of(new B())).get(0).run();
  }

  int useClassHashSet() {
    return new HashSet<>(List.of(new A())).iterator().next().execute()
