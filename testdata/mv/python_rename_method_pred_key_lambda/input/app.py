...
def use_takewhile(items: list[A]):
    takewhile(lambda x: x.run(), items)


def use_takewhile_mod(items: list[A]):
    itertools.takewhile(lambda x: x.run(), items)
...
def use_dropwhile(items: list[A]):
    dropwhile(lambda x: x.run(), items)


def use_dropwhile_mod(items: list[A]):
    itertools.dropwhile(lambda x: x.run(), items)
...
def use_filterfalse(items: list[A]):
    filterfalse(lambda x: x.run(), items)


def use_filterfalse_mod(items: list[A]):
    itertools.filterfalse(lambda x: x.run(), items)
...
def use_nlargest(items: list[A]):
    nlargest(1, items, key=lambda x: x.run())


def use_nlargest_mod(items: list[A]):
    heapq.nlargest(1, items, key=lambda x: x.run())
...
def use_nsmallest(items: list[A]):
    nsmallest(1, items, key=lambda x: x.run())


def use_nsmallest_mod(items: list[A]):
    heapq.nsmallest(1, items, key=lambda x: x.run())
...
def use_takewhile_assigned():
    xs = [A()]
    takewhile(lambda x: x.run(), xs)
    ys = [B()]
    takewhile(lambda y: y.run(), ys)


def use_nlargest_literal():
    nlargest(1, [A()], key=lambda x: x.run())
