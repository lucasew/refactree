...
def use_dict_multi(ba: BoxA, bb: BoxB) -> int:
    return {"k": ba.get(), "j": ba.get()}["j"].run() + {"k": bb.get(), "j": bb.get()}["j"].run()


def use_dict_typed_local(ba: BoxA, bb: BoxB) -> int:
    xa = ba.get()
    xb = bb.get()
    return {"k": xa}["k"].run() + {"k": xb}["k"].run()


def use_dict_walrus(ba: BoxA, bb: BoxB) -> int:
    return (da := {"k": ba.get()})["k"].run() + (db := {"k": bb.get()})["k"].run()


def use_list_concat_inline(ba: BoxA, bb: BoxB) -> int:
    return ([ba.get()] + [ba.get()])[0].run() + ([bb.get()] + [bb.get()])[0].run()
...
def use_list_concat_chain(ba: BoxA, bb: BoxB) -> int:
    return ([ba.get()] + [ba.get()] + [ba.get()])[0].run() + ([bb.get()] + [bb.get()] + [bb.get()])[0].run()


def use_list_ctor_inline(ba: BoxA, bb: BoxB) -> int:
    return list([ba.get()])[0].run() + list([bb.get()])[0].run()
...
def use_tuple_ctor(ba: BoxA, bb: BoxB) -> int:
    return tuple((ba.get(),))[0].run() + tuple((bb.get(),))[0].run()


def use_list_or_inline(ba: BoxA, bb: BoxB) -> int:
    return ([ba.get()] or [ba.get()])[0].run() + ([bb.get()] or [bb.get()])[0].run()
...
def use_list_and_inline(ba: BoxA, bb: BoxB) -> int:
    return ([ba.get()] and [ba.get()])[0].run() + ([bb.get()] and [bb.get()])[0].run()


def use_list_or_empty(ba: BoxA, bb: BoxB) -> int:
    return ([ba.get()] or [])[0].run() + ([bb.get()] or [])[0].run()
