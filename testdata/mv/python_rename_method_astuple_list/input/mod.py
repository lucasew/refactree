...
def use_list(box: Box) -> int:
    return list(astuple(box))[0].run() + list(astuple(box))[1].run()


def use_tuple(box: Box) -> int:
    return tuple(astuple(box))[0].run() + tuple(astuple(box))[1].run()


def use_dc_list(box: Box) -> int:
    return list(dataclasses.astuple(box))[0].run() + list(dataclasses.astuple(box))[1].run()


def use_list_replace(box: Box) -> int:
    return list(astuple(replace(box)))[0].run() + list(astuple(replace(box)))[1].run()


def use_var(box: Box) -> int:
    xs = list(astuple(box))
    return xs[0].run() + xs[1].run()
