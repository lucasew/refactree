...
class Use {
  ...
  int useAssign(BoxA ba, BoxB bb) {
    ...
    return ar.get().run() + br.get().run();
  }

  int useClass() {
    return new AtomicReference<>(new A()).get().run()
