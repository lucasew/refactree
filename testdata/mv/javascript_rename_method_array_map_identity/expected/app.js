...
function useMapIndex() {
  return [new A()].map((x) => x)[0].execute() + [new B()].map((x) => x)[0].run();
}
...
function useMapAt() {
  return (
    [new A()].map((x) => x).at(0).execute() + [new B()].map((x) => x).at(0).run()
  );
}
...
function useArrayFromMap() {
  return (
    Array.from([new A()]).map((x) => x)[0].execute() +
...
function useIdent() {
  const a = new A();
  const b = new B();
  return [a].map((x) => x)[0].execute() + [b].map((x) => x)[0].run();
}
