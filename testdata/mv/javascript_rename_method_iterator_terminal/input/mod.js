...
function useFlatMapNext() {
  return (
    Iterator.from([0]).flatMap(() => [new A()]).next().value.run() +
...
function useFlatMapForOf() {
  let s = 0;
  for (const x of Iterator.from([0]).flatMap(() => [new A()])) s += x.run();
...
function useReduce() {
  return (
    Iterator.from([new A()]).reduce((a, x) => x).run() +
...
function useForEach() {
  Iterator.from([new A()]).forEach((x) => x.run());
  Iterator.from([new B()]).forEach((y) => y.run());
}

function useSomeEvery() {
  return (
    Iterator.from([new A()]).some((x) => x.run() > 0) +
    Iterator.from([new B()]).some((y) => y.run() > 0) +
    Iterator.from([new A()]).every((x) => x.run() > 0) +
