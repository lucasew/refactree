...
function useObjectValuesNested() {
  return Object.values({k: [new A()]})[0][0].execute()
...
function useObjectValuesNestedLocal() {
  const oa = {k: [new A()]};
  const ob = {k: [new B()]};
  return Object.values(oa)[0][0].execute() + Object.values(ob)[0][0].run();
}
...
function useNestedFlatMapLocal() {
  const aa = [[new A()]];
  const bb = [[new B()]];
  return aa.flatMap((xs) => xs)[0].execute() + bb.flatMap((xs) => xs)[0].run();
}
