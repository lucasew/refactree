...
function useMapEntriesSpread() {
  return (
    [...new Map([["k", new A()]]).entries()][0][1].execute() +
...
function useMapEntriesNextValuePair() {
  return (
    new Map([["k", new A()]]).entries().next().value[1].execute() +
...
function useMultiPair() {
  return (
    [...new Map([["k", new A()], ["j", new A()]]).entries()][1][1].execute() +
...
function useMapValuesNext() {
  return (
    new Map([["k", new A()]]).values().next().value.execute() +
