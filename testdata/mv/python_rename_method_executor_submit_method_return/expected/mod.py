...
def use_class_inline() -> int:
    with ThreadPoolExecutor() as ex:
        return (
            ex.submit(lambda: A()).result().execute()
...
def use_class_timeout() -> int:
    with ThreadPoolExecutor() as ex:
        return (
            ex.submit(lambda: A()).result(timeout=1).execute()
...
def use_mr_inline(ba: BoxA, bb: BoxB) -> int:
    with ThreadPoolExecutor() as ex:
        return (
            ex.submit(lambda: ba.get()).result().execute()
...
def use_mr_timeout(ba: BoxA, bb: BoxB) -> int:
    with ThreadPoolExecutor() as ex:
        return (
            ex.submit(lambda: ba.get()).result(timeout=1).execute()
