...
def use_list_multi(ba: BoxA, bb: BoxB) -> int:
    return [ba.get(), ba.get()][1].execute() + [bb.get(), bb.get()][1].run()
...
def use_list_chain(ba: BoxA, bb: BoxB) -> int:
    return [ba.self().get()][0].execute() + [bb.self().get()][0].run()
...
def use_list_walrus(ba: BoxA, bb: BoxB) -> int:
    return (xs := [ba.get()])[0].execute() + (ys := [bb.get()])[0].run()


def use_list_ternary(ba: BoxA, bb: BoxB, c: bool) -> int:
    return [(ba.get() if c else ba.get())][0].execute() + [(bb.get() if c else bb.get())][0].run()


def use_or_method(ba: BoxA, bb: BoxB) -> int:
    return (ba.get() or ba.get()).execute() + (bb.get() or bb.get()).run()


def use_and_method(ba: BoxA, bb: BoxB) -> int:
    return (ba.get() and ba.get()).execute() + (bb.get() and bb.get()).run()


def use_or_chain(ba: BoxA, bb: BoxB) -> int:
    return (ba.get() or ba.get() or ba.get()).execute() + (bb.get() or bb.get() or bb.get()).run()


def use_or_typed(ba: BoxA, bb: BoxB) -> int:
    xa = ba.get()
    xb = bb.get()
    return (xa or xa).execute() + (xb or xb).run()


def use_and_typed(ba: BoxA, bb: BoxB) -> int:
    xa = ba.get()
    xb = bb.get()
    return (xa and xa).execute() + (xb and xb).run()
