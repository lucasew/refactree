...
class Use {
  int useOrElseGet(BoxA ba, BoxB bb) {
    return Optional.<A>empty().orElseGet(() -> ba.get()).run()
  ...
  int useOrElse(BoxA ba, BoxB bb) {
    return Optional.<A>empty().orElse(ba.get()).run()
  ...
  int useGenerate(BoxA ba, BoxB bb) {
    return Stream.generate(() -> ba.get()).limit(1).findFirst().get().run()
  ...
  int useClass() {
    return Optional.<A>empty().orElseGet(() -> new A()).run()
        ...
        + Optional.<A>empty().orElse(new A()).run()
        + Optional.<B>empty().orElse(new B()).run()
        + Stream.generate(() -> new A()).limit(1).findFirst().get().run()
        + Stream.generate(() -> new B()).limit(1).findFirst().get().run()
        + Stream.iterate(new A(), x -> x).limit(1).findFirst().get().run()
        + Stream.iterate(new B(), x -> x).limit(1).findFirst().get().run()
        + CompletableFuture.supplyAsync(() -> new A()).join().run()
        + CompletableFuture.supplyAsync(() -> new B()).join().run()
        + ThreadLocal.withInitial(() -> new A()).get().run()
