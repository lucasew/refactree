...
function useMapThenFlatLocal() {
  const aa = [[new A()]];
  const bb = [[new B()]];
  return aa.map((xs) => xs).flat()[0].run()
...
function useMapThenFlatInline() {
  return [[new A()]].map((xs) => xs).flat()[0].run()
...
function useMapIdentityThenFlat() {
  const aa = [[new A()]];
  const bb = [[new B()]];
  return aa.map((xs) => xs.map((x) => x)).flat()[0].run()
...
function useMapThenFlatDepth1() {
  const aa = [[new A()]];
  const bb = [[new B()]];
  return aa.map((xs) => xs).flat(1)[0].run()
...
function useMapLocalThenFlat() {
  ...
  return ma.flat()[0].run() + mb.flat()[0].run();
}
