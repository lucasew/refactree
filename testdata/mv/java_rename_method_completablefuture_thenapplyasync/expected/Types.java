...
class Uses {
  public static int useThenApplyAsyncJoin() {
    return CompletableFuture.completedFuture(new A()).thenApplyAsync(a -> a).join().execute()
  ...
  public static int useThenApplyAsyncExecutor() {
    ...
    return CompletableFuture.completedFuture(new A()).thenApplyAsync(a -> a, ex).join().execute()
