...
class Use {
  ...
  int useVar(BoxA ba, BoxB bb) {
    ...
    return aa[0].run() + ab[0].run();
  }

  // Class regression — already worked.
  int useClass() {
    return Stream.of(new A()).toArray(A[]::new)[0].run()
        + Stream.of(new B()).toArray(B[]::new)[0].run()
        + List.of(new A()).toArray(A[]::new)[0].run()
        + List.of(new B()).toArray(B[]::new)[0].run()
        + Stream.of(new A()).toArray(new A[0])[0].run()
