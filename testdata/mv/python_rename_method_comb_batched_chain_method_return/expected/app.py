...
def use_comb_direct(ba: BoxA, bb: BoxB) -> int:
    return (
        list(combinations([ba.get(), ba.get()], 2))[0][0].execute()
...
def use_perm_direct(ba: BoxA, bb: BoxB) -> int:
    return (
        list(permutations([ba.get(), ba.get()], 2))[0][0].execute()
...
def use_cwr_direct(ba: BoxA, bb: BoxB) -> int:
    return (
        list(combinations_with_replacement([ba.get()], 2))[0][0].execute()
...
def use_batched_direct(ba: BoxA, bb: BoxB) -> int:
    return (
        list(batched([ba.get(), ba.get()], 2))[0][0].execute()
...
def use_batched_itertools(ba: BoxA, bb: BoxB) -> int:
    return (
        list(itertools.batched([ba.get(), ba.get()], 2))[0][0].execute()
...
def use_chain_from_iterable(ba: BoxA, bb: BoxB) -> int:
    return (
        list(chain.from_iterable([[ba.get()]]))[0].execute()
...
def use_chain_from_iterable_itertools(ba: BoxA, bb: BoxB) -> int:
    return (
        list(itertools.chain.from_iterable([[ba.get()]]))[0].execute()
...
def use_comb_class() -> int:
    return (
        list(combinations([A(), A()], 2))[0][0].execute()
        + list(combinations([B(), B()], 2))[0][0].run()
        + list(batched([A(), A()], 2))[0][0].execute()
        + list(chain.from_iterable([[A()]]))[0].execute()
