...
class Uses {
  public static int useDirect(List<A> as, List<B> bs) {
    return as.stream().toArray()[0].run() + bs.stream().toArray()[0].run();
  }

  public static int useTyped(List<A> as, List<B> bs) {
    return as.stream().toArray(new A[0])[0].run() + bs.stream().toArray(new B[0])[0].run();
  }
  ...
  public static int useListToArray(List<A> as, List<B> bs) {
    return as.toArray(new A[0])[0].run() + bs.toArray(new B[0])[0].run();
  }
