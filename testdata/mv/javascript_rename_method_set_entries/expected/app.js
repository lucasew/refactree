...
function useEntriesNextValuePair() {
  return (
    new Set([new A()]).entries().next().value[1].execute() +
...
function useSetLocalEntries() {
  ...
  return sa.entries().next().value[1].execute() + sb.entries().next().value[1].run();
}
...
function useEntriesSpread() {
  return (
    [...new Set([new A()]).entries()][0][1].execute() +
...
function useIdent() {
  ...
  const b = new B();
  return (
    new Set([a]).entries().next().value[1].execute() +
...
function useArrayLocal() {
  ...
  const bs = [new B()];
  return (
    new Set(as).entries().next().value[1].execute() +
