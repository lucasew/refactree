...
function useFill() {
  return (
    [new A()].fill(new A())[0].run() + [new B()].fill(new B())[0].run()
  );
}

function useFillRange() {
  return (
    [new A()].fill(new A(), 0, 1)[0].run() +
...
function useFillIdent() {
  const a = new A();
  const b = new B();
  return [new A()].fill(a)[0].run() + [new B()].fill(b)[0].run();
}

function useCopyWithin() {
  return (
    [new A()].copyWithin(0)[0].run() + [new B()].copyWithin(0)[0].run()
  );
}

function useCopyWithinArgs() {
  return (
    [new A()].copyWithin(0, 0)[0].run() + [new B()].copyWithin(0, 0)[0].run()
  );
}
