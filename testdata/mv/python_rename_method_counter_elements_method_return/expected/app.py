...
def use_elements(ba: BoxA, bb: BoxB):
    return list(Counter([ba.get()]).elements())[0].execute() + list(
        Counter([bb.get()]).elements()
    )[0].run()


def use_collections_counter(ba: BoxA, bb: BoxB):
    return list(Counter([ba.get()]))[0].execute() + list(Counter([bb.get()]))[0].run()
...
def use_class():
    return list(Counter([A()]).elements())[0].execute() + list(
        Counter([B()]).elements()
    )[0].run()
