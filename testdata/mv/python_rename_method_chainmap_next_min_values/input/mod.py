...
def use_chainmap_sub(ba: BoxA, bb: BoxB) -> int:
    return ChainMap({"k": ba.get()})["k"].run() + ChainMap({"k": bb.get()})["k"].run()


def use_chainmap_get(ba: BoxA, bb: BoxB) -> int:
    return ChainMap({"k": ba.get()}).get("k").run() + ChainMap({"k": bb.get()}).get("k").run()
...
def use_chainmap_dict_kw(ba: BoxA, bb: BoxB) -> int:
    return ChainMap(dict(k=ba.get()))["k"].run() + ChainMap(dict(k=bb.get()))["k"].run()


def use_chainmap_multi(ba: BoxA, bb: BoxB) -> int:
    return (
        ChainMap({"k": ba.get()}, {"j": ba.get()})["j"].run()
        + ChainMap({"k": bb.get()}, {"j": bb.get()})["j"].run()
    )


def use_next_iter_list(ba: BoxA, bb: BoxB) -> int:
    return next(iter([ba.get()])).run() + next(iter([bb.get()])).run()


def use_next_list(ba: BoxA, bb: BoxB) -> int:
    return next([ba.get()]).run() + next([bb.get()]).run()
...
def use_next_walrus(ba: BoxA, bb: BoxB) -> int:
    return (xa := next(iter([ba.get()]))).run() + (xb := next(iter([bb.get()]))).run()


def use_next_dict_values(ba: BoxA, bb: BoxB) -> int:
    return (
        next(iter(dict(k=ba.get()).values())).run()
        + next(iter(dict(k=bb.get()).values())).run()
    )


def use_min_list(ba: BoxA, bb: BoxB) -> int:
    return min([ba.get()], key=lambda x: 0).run() + min([bb.get()], key=lambda x: 0).run()


def use_max_list(ba: BoxA, bb: BoxB) -> int:
    return max([ba.get()], key=lambda x: 0).run() + max([bb.get()], key=lambda x: 0).run()
...
def use_min_dict_values(ba: BoxA, bb: BoxB) -> int:
    return (
        min(dict(k=ba.get()).values(), key=lambda x: 0).run()
...
def use_choice_list(ba: BoxA, bb: BoxB) -> int:
    return choice([ba.get()]).run() + choice([bb.get()]).run()


def use_heappop_list(ba: BoxA, bb: BoxB) -> int:
    return heappop([ba.get()]).run() + heappop([bb.get()]).run()


def use_reduce_list(ba: BoxA, bb: BoxB) -> int:
    return reduce(lambda a, b: a, [ba.get()]).run() + reduce(lambda a, b: a, [bb.get()]).run()


def use_max_chainmap_values(ba: BoxA, bb: BoxB) -> int:
    return (
        max(ChainMap({"k": ba.get()}).values(), key=lambda x: 0).run()
