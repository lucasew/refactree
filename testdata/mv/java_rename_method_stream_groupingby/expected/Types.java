...
class Uses {
  public static int useValuesForEach(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.groupingBy(a -> "k")).values().forEach(g -> g.forEach(a -> a.execute()));
  ...
  public static int useMapForEach(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.groupingBy(a -> "k")).forEach((k, g) -> g.forEach(a -> a.execute()));
