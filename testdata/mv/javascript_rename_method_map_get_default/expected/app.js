...
function useNullishGet() {
  ...
  ma.set("k", new A());
  mb.set("k", new B());
  return (ma.get("k") ?? new A()).execute() + (mb.get("k") ?? new B()).run();
}
...
function useNullishNullDefault() {
  const ma = new Map([["k", new A()]]);
  const mb = new Map([["k", new B()]]);
  return (ma.get("k") ?? null).execute() + (mb.get("k") ?? null).run();
}

function useNullishUndefinedDefault() {
  const ma = new Map([["k", new A()]]);
  const mb = new Map([["k", new B()]]);
  return (ma.get("k") ?? undefined).execute() + (mb.get("k") ?? undefined).run();
}
...
function useWeakMapNullish() {
  ...
  ma.set(ka, new A());
  mb.set(kb, new B());
  return (ma.get(ka) ?? new A()).execute() + (mb.get(kb) ?? new B()).run();
}
