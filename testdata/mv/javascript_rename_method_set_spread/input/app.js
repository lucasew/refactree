...
function useArrayFromLocal() {
  const xs = new Set([new A()]);
  const ys = new Set([new B()]);
  return Array.from(xs)[0].run() + Array.from(ys)[0].run();
}

function useValuesSpread() {
  const xs = new Set([new A()]);
  const ys = new Set([new B()]);
  return [...xs.values()][0].run() + [...ys.values()][0].run();
}
...
function useAddSpread() {
  ...
  xs.add(new A());
  ys.add(new B());
  return [...xs][0].run() + [...ys][0].run() + [...xs.values()][0].run() + [...ys.values()][0].run();
}
...
function useInlineNewSet() {
  return (
    [...new Set([new A()])][0].run() +
    [...new Set([new B()])][0].run() +
    Array.from(new Set([new A()]))[0].run() +
