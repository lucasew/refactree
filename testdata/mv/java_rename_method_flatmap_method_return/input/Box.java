...
class Use {
  int useStreamFlatMap(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).flatMap(x -> Stream.of(x)).findFirst().get().run()
  ...
  int useOptFlatMap(BoxA ba, BoxB bb) {
    return Optional.of(ba.get()).flatMap(x -> Optional.of(x)).get().run()
  ...
  int useClass() {
    return Stream.of(new A()).flatMap(x -> Stream.of(x)).findFirst().get().run()
        + Stream.of(new B()).flatMap(x -> Stream.of(x)).findFirst().get().run()
        + Optional.of(new A()).flatMap(x -> Optional.of(x)).get().run()
