...
def use_class_nested_list() -> int:
    return [[A()]][0][0].execute() + [[B()]][0][0].run()
...
def use_class_list_of_dict() -> int:
    return [{"k": A()}][0]["k"].execute() + [{"k": B()}][0]["k"].run()


def use_class_dict_of_list() -> int:
    return {"k": [A()]}["k"][0].execute() + {"k": [B()]}["k"][0].run()


def use_class_dict_of_dict() -> int:
    return {"o": {"k": A()}}["o"]["k"].execute() + {"o": {"k": B()}}["o"]["k"].run()


def use_class_assign_nested() -> int:
    xs = [[A()]]
    ys = [[B()]]
    return xs[0][0].execute() + ys[0][0].run()
...
def use_mr_nested_list(ba: BoxA, bb: BoxB) -> int:
    return [[ba.get()]][0][0].execute() + [[bb.get()]][0][0].run()
...
def use_mr_list_of_dict(ba: BoxA, bb: BoxB) -> int:
    return [{"k": ba.get()}][0]["k"].execute() + [{"k": bb.get()}][0]["k"].run()


def use_mr_dict_of_list(ba: BoxA, bb: BoxB) -> int:
    return {"k": [ba.get()]}["k"][0].execute() + {"k": [bb.get()]}["k"][0].run()


def use_mr_dict_of_dict(ba: BoxA, bb: BoxB) -> int:
    return {"o": {"k": ba.get()}}["o"]["k"].execute() + {"o": {"k": bb.get()}}["o"]["k"].run()
