...
class Uses {
  ...
  public static int useDescendingMapGetChain(
    ...
    return am.descendingMap().get("k").execute() + bm.descendingMap().get("k").run();
  }

  public static int useHeadMapGetChain(
      NavigableMap<String, A> am, NavigableMap<String, B> bm) {
    return am.headMap("z").get("k").execute() + bm.headMap("z").get("k").run();
  }

  public static int useTailMapGetChain(
      NavigableMap<String, A> am, NavigableMap<String, B> bm) {
    return am.tailMap("a").get("k").execute() + bm.tailMap("a").get("k").run();
  }

  public static int useSubMapGetChain(
      NavigableMap<String, A> am, NavigableMap<String, B> bm) {
    return am.subMap("a", "z").get("k").execute() + bm.subMap("a", "z").get("k").run();
  }

  // NavigableMap overloads with inclusivity flags (bounds only; same V).
  public static int useHeadMapInclusiveChain(
      NavigableMap<String, A> am, NavigableMap<String, B> bm) {
    return am.headMap("z", true).get("k").execute() + bm.headMap("z", true).get("k").run();
  }

  public static int useTailMapInclusiveChain(
      NavigableMap<String, A> am, NavigableMap<String, B> bm) {
    return am.tailMap("a", false).get("k").execute() + bm.tailMap("a", false).get("k").run();
  }

  public static int useSubMapInclusiveChain(
      NavigableMap<String, A> am, NavigableMap<String, B> bm) {
    return am.subMap("a", true, "z", false).get("k").execute()
  ...
  public static int useHeadMapForEach(
      NavigableMap<String, A> am, NavigableMap<String, B> bm) {
    am.headMap("z").forEach((k, a) -> a.execute());
  ...
  public static int useSubMapEntrySet(
      NavigableMap<String, A> am, NavigableMap<String, B> bm) {
    am.subMap("a", "z").entrySet().forEach(ea -> ea.getValue().execute());
