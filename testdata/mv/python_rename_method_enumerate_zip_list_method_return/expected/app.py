...
def use_enum_direct(ba: BoxA, bb: BoxB) -> int:
    return (
        list(enumerate([ba.get()]))[0][1].execute()
...
def use_enum_pair(ba: BoxA, bb: BoxB) -> int:
    ...
    return pair_a[1].execute() + pair_b[1].run()
...
def use_next_enum_sub(ba: BoxA, bb: BoxB) -> int:
    return (
        next(enumerate([ba.get()]))[1].execute()
...
def use_tuple_zip(ba: BoxA, bb: BoxB) -> int:
    return (
        tuple(zip([ba.get()], [0]))[0][0].execute()
...
def use_min_zip(ba: BoxA, bb: BoxB) -> int:
    return (
        min(list(zip([ba.get()], [0])))[0].execute()
...
def use_max_zip(ba: BoxA, bb: BoxB) -> int:
    return (
        max(list(zip([ba.get()], [0])))[0].execute()
...
def use_choice_zip(ba: BoxA, bb: BoxB) -> int:
    return (
        random.choice(list(zip([ba.get()], [0])))[0].execute()
...
def use_heappop_zip(ba: BoxA, bb: BoxB) -> int:
    return (
        heapq.heappop(list(zip([ba.get()], [0])))[0].execute()
...
def use_pop_zip(ba: BoxA, bb: BoxB) -> int:
    return (
        list(zip([ba.get()], [0])).pop()[0].execute()
...
def use_itemgetter_zip(ba: BoxA, bb: BoxB) -> int:
    return (
        itemgetter(0)(list(zip([ba.get()], [0])))[0].execute()
...
def use_product(ba: BoxA, bb: BoxB) -> int:
    return (
        list(product([ba.get()], [0]))[0][0].execute()
...
def use_pairwise(ba: BoxA, bb: BoxB) -> int:
    return (
        list(pairwise([ba.get(), ba.get()]))[0][0].execute()
...
def use_zip_longest(ba: BoxA, bb: BoxB) -> int:
    return (
        list(zip_longest([ba.get()], [0]))[0][0].execute()
...
def use_reversed_zip(ba: BoxA, bb: BoxB) -> int:
    return (
        list(reversed(list(zip([ba.get()], [0]))))[0][0].execute()
...
def use_class() -> int:
    return (
        list(enumerate([A()]))[0][1].execute()
        + list(enumerate([B()]))[0][1].run()
        + list(zip([A()], [0]))[0][0].execute()
        + list(zip([B()], [0]))[0][0].run()
        + min(list(zip([A()], [0])))[0].execute()
