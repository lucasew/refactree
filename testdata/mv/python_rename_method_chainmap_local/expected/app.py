...
def use_nested_inline():
    ...
    return ChainMap(na)["k"][0].execute() + ChainMap(nb)["k"][0].run()
...
def use_nested_multi():
    ...
    return ChainMap(na, pa)["j"][0].execute() + ChainMap(nb, pb)["j"][0].run()
...
def use_nested_od():
    oa = OrderedDict(k=[A()])
    ob = OrderedDict(k=[B()])
    return ChainMap(oa)["k"][0].execute() + ChainMap(ob)["k"][0].run()


def use_collections_chainmap():
    na: dict[str, list[A]] = {"k": [A()]}
    nb: dict[str, list[B]] = {"k": [B()]}
    return collections.ChainMap(na)["k"][0].execute() + collections.ChainMap(nb)["k"][0].run()
...
def use_scalar_inline():
    sa: dict[str, A] = {"k": A()}
    sb: dict[str, B] = {"k": B()}
    return ChainMap(sa)["k"].execute() + ChainMap(sb)["k"].run()
