...
def use_next_iter_values():
    return (
        next(iter(vars(SimpleNamespace(k=A())).values())).run()
...
def use_next_values():
    return (
        next(vars(SimpleNamespace(k=A())).values()).run()
...
def use_next_iter_types():
    return (
        next(iter(vars(types.SimpleNamespace(k=A())).values())).run()
...
def use_next_iter_dunder():
    return (
        next(iter(SimpleNamespace(k=A()).__dict__.values())).run()
...
def use_list_values_index():
    return (
        list(vars(SimpleNamespace(k=A())).values())[0].run()
...
def use_tuple_values_index():
    return (
        tuple(vars(SimpleNamespace(k=A())).values())[0].run()
