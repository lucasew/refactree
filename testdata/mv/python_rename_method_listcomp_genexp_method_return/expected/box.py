...
def use_listcomp_inline_mr(ba: BoxA, bb: BoxB) -> int:
    return [ba.get() for _ in range(1)][0].execute() + [bb.get() for _ in range(1)][0].run()


def use_listcomp_inline_class() -> int:
    return [A() for _ in range(1)][0].execute() + [B() for _ in range(1)][0].run()


def use_genexp_next_mr(ba: BoxA, bb: BoxB) -> int:
    xs = (ba.get() for _ in range(1))
    ys = (bb.get() for _ in range(1))
    return next(xs).execute() + next(ys).run()
...
def use_genexp_inline_mr(ba: BoxA, bb: BoxB) -> int:
    return next(ba.get() for _ in range(1)).execute() + next(bb.get() for _ in range(1)).run()


def use_genexp_inline_class() -> int:
    return next(A() for _ in range(1)).execute() + next(B() for _ in range(1)).run()
