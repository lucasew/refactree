...
class Uses {
  public static int useFlatMappingForEach(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.flatMapping(a -> Stream.of(a), Collectors.toList())).forEach(a -> a.run());
  ...
  public static int useFlatMappingMethodRef(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.flatMapping(Stream::of, Collectors.toList())).forEach(a -> a.run());
  ...
  public static int useFlatMappingOfNullable(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.flatMapping(a -> Stream.ofNullable(a), Collectors.toList())).forEach(a -> a.run());
  ...
  public static int useFlatMappingToSet(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.flatMapping(a -> Stream.of(a), Collectors.toSet())).forEach(a -> a.run());
