...
function useFind() {
  return (
    Iterator.from([new BoxA().get()]).find((x) => true).run() +
...
function useFindLast() {
  return (
    Iterator.from([new BoxA().get()]).findLast((x) => true).run() +
...
function useReduce() {
  return (
    Iterator.from([new BoxA().get()]).reduce((a, x) => a).run() +
...
function useArrayFlatMap() {
  return (
    [new BoxA().get()].flatMap((x) => [x])[0].run() +
...
function useArrayReduceInit() {
  return (
    [new BoxA().get()].reduce((a, b) => a, new BoxA().get()).run() +
