...
def use_class_body() -> int:
    return make_ca().run() + make_cb().run()


def use_mr_body(ba: BoxA, bb: BoxB) -> int:
    return make_ma(ba).run() + make_mb(bb).run()


def use_class_body_assign() -> int:
    return make_caa().run() + make_cab().run()


def use_mr_body_assign(ba: BoxA, bb: BoxB) -> int:
    return make_maa(ba).run() + make_mab(bb).run()


def use_class_call_assign() -> int:
    cya = make_ca()
    cyb = make_cb()
    return cya.run() + cyb.run()
...
def use_class_nested() -> int:
    ...
    return make_cna().run() + make_cnb().run()
...
def use_mr_nested(ba: BoxA, bb: BoxB) -> int:
    ...
    return make_mna().run() + make_mnb().run()
...
def use_class_ann() -> int:
    return make_ca_ann().run() + make_cb_ann().run()


def use_mr_ann(ba: BoxA, bb: BoxB) -> int:
    return make_ma_ann(ba).run() + make_mb_ann(bb).run()
