...
class Use {
  ...
  int useMapCopyOf(BoxA ba, BoxB bb) {
    return Map.copyOf(Map.of("k", ba.get())).get("k").run()
  ...
  int useSubList(BoxA ba, BoxB bb) {
    return List.of(ba.get()).subList(0, 1).get(0).run()
  ...
  int useSubListVar(BoxA ba, BoxB bb) {
    ...
    return xa.get(0).run() + xb.get(0).run();
  }

  // Class regression — already worked.
  int useClass() {
    return Map.copyOf(Map.of("k", new A())).get("k").run()
        + Map.copyOf(Map.of("k", new B())).get("k").run()
        + Collections.unmodifiableMap(Map.of("k", new A())).get("k").run()
        + Collections.unmodifiableMap(Map.of("k", new B())).get("k").run()
        + List.of(new A()).subList(0, 1).get(0).run()
