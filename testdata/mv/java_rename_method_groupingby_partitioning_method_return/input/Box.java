...
class Use {
  ...
  int useGroupingByGet(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).collect(Collectors.groupingBy(a -> "k")).get("k").get(0).run()
  ...
  int useGroupingByListVar(BoxA ba, BoxB bb) {
    ...
    return ga.get(0).run() + gb.get(0).run();
  }

  int useGroupingByDownstream(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).collect(Collectors.groupingBy(a -> "k", Collectors.toList())).get("k").get(0).run()
  ...
  int useGroupingByEntry(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).collect(Collectors.groupingBy(a -> "k")).entrySet().iterator().next().getValue().get(0).run()
  ...
  int useGroupingByValuesForEach(BoxA ba, BoxB bb) {
    int[] n = {0};
    Stream.of(ba.get()).collect(Collectors.groupingBy(a -> "k")).values().forEach(g -> g.forEach(xa -> n[0] += xa.run()));
  ...
  int usePartitioningByGet(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).collect(Collectors.partitioningBy(a -> true)).get(true).get(0).run()
  ...
  int usePartitioningByDownstream(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).collect(Collectors.partitioningBy(a -> true, Collectors.toList())).get(true).get(0).run()
  ...
  int usePartitioningByVar(BoxA ba, BoxB bb) {
    ...
    return ma.get(true).get(0).run() + mb.get(true).get(0).run();
  }

  // Class regression — already worked.
  int useClass() {
    return Stream.of(new A()).collect(Collectors.groupingBy(a -> "k")).get("k").get(0).run()
        + Stream.of(new B()).collect(Collectors.groupingBy(b -> "k")).get("k").get(0).run()
        + Stream.of(new A()).collect(Collectors.partitioningBy(a -> true)).get(true).get(0).run()
        + Stream.of(new B()).collect(Collectors.partitioningBy(b -> true)).get(true).get(0).run()
        + Stream.of(new A()).collect(Collectors.groupingBy(a -> "k")).entrySet().iterator().next().getValue().get(0).run()
