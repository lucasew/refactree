...
func UseNamed(sa SA, sb SB) int {
	return sa.Named[0].Execute() + sb.Named[0].Run()
}

func UseMap(sa SA, sb SB) int {
	return sa.M["k"].Execute() + sb.M["k"].Run()
}

func UseNamedMap(sa SA, sb SB) int {
	return sa.NamedM["k"].Execute() + sb.NamedM["k"].Run()
}
...
func UseEmbed(wa WA, wb WB) int {
	return wa.Items[0].Execute() + wb.Items[0].Run()
}

func UseEmbedMap(wa WA, wb WB) int {
	return wa.M["k"].Execute() + wb.M["k"].Run()
}
