...
def use_groupby_key(items: list[A]):
    groupby(items, key=lambda x: x.execute())


def use_groupby_key_mod(items: list[A]):
    itertools.groupby(items, key=lambda x: x.execute())
...
def use_groupby_key_for(items: list[A]):
    for k, g in groupby(items, key=lambda x: x.execute()):
        for a in g:
            a.execute()
