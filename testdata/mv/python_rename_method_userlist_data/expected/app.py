...
def use_data_sub():
    xs = UserList([A()])
    ys = UserList([B()])
    return xs.data[0].execute() + ys.data[0].run()
...
def use_userdict_data():
    da = UserDict(k=A())
    db = UserDict(k=B())
    return da.data["k"].execute() + db.data["k"].run()
...
def use_userdict_nested_data():
    da = UserDict(k=[A()])
    db = UserDict(k=[B()])
    return da.data["k"][0].execute() + db.data["k"][0].run()
...
def use_userdict_nested_data_local():
    ...
    xa = da.data
    xb = db.data
    return xa["k"][0].execute() + xb["k"][0].run()

def use_data_pop():
    xs = UserList([A()])
    ys = UserList([B()])
    return xs.data.pop().execute() + ys.data.pop().run()
