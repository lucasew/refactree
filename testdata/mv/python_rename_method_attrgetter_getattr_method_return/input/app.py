...
def use_attrgetter(w: Wrap, wb: WrapB) -> int:
    return attrgetter("a")(w.get()).run() + attrgetter("a")(wb.get()).run()
...
def use_getattr(w: Wrap, wb: WrapB) -> int:
    return getattr(w.get(), "a").run() + getattr(wb.get(), "a").run()
...
def use_class() -> int:
    return attrgetter("a")(Box(A())).run() + attrgetter("a")(BoxB(B())).run()


def use_typed(box: Box, boxb: BoxB) -> int:
    return attrgetter("a")(box).run() + getattr(boxb, "a").run()
