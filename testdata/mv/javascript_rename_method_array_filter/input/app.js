...
function useFilterBare() {
  return (
    [new A()].filter(x => x)[0].run() + [new B()].filter(x => x)[0].run()
  );
}
...
function useFilterSlice() {
  return (
    [new A()].slice().filter((x) => true)[0].run() +
...
function useArrayFromFilter() {
  return (
    Array.from([new A()]).filter((x) => true)[0].run() +
...
function useIdent() {
  const a = new A();
  const b = new B();
  return [a].filter((x) => true)[0].run() + [b].filter((x) => true)[0].run();
}
