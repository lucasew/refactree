...
class Uses {
  public static int useConcurrentValues(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.toConcurrentMap(a -> "k", a -> a)).values().forEach(a -> a.run());
  ...
  public static int useConcurrentMerge(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.toConcurrentMap(a -> "k", a -> a, (x, y) -> x)).values().forEach(a -> a.run());
  ...
  public static int useUnmodifiableValues(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.toUnmodifiableMap(a -> "k", a -> a)).values().forEach(a -> a.run());
  ...
  public static int useUnmodifiableMerge(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.toUnmodifiableMap(a -> "k", a -> a, (x, y) -> x)).values().forEach(a -> a.run());
