...
def use_copy_next_values():
    return (
        copy.copy(next(iter(vars(SimpleNamespace(k=A())).values()))).execute()
...
def use_copy_next_direct():
    return (
        copy.copy(next(vars(SimpleNamespace(k=A())).values())).execute()
...
def use_copy_next_types():
    return (
        copy.copy(next(iter(vars(types.SimpleNamespace(k=A())).values()))).execute()
...
def use_copy_next_dunder():
    return (
        copy.copy(next(iter(SimpleNamespace(k=A()).__dict__.values()))).execute()
...
def use_deepcopy():
    return (
        copy.deepcopy(next(iter(vars(SimpleNamespace(k=A())).values()))).execute()
...
def use_copy_list_index():
    return (
        copy.copy(list(vars(SimpleNamespace(k=A())).values())[0]).execute()
...
def use_copy_getattr():
    return (
        copy.copy(getattr(SimpleNamespace(k=A()), "k")).execute()
...
def use_copy_getattr_types():
    return (
        copy.copy(getattr(types.SimpleNamespace(k=A()), "k")).execute()
