...
func UseMakeInline() int {
	return make(AS, 1)[0].Execute() + make(BS, 1)[0].Run()
}
...
func UseCompositeInline() int {
	return AS{&A{}}[0].Execute() + BS{&B{}}[0].Run()
}

func UseCompositeMap() int {
	return AM{"k": &A{}}["k"].Execute() + BM{"k": &B{}}["k"].Run()
}
...
func UseAssertInline(x, y any) int {
	return x.(AS)[0].Execute() + y.(BS)[0].Run()
}
...
func UseConvertInline(x, y any) int {
	return (AS)(x)[0].Execute() + (BS)(y)[0].Run()
}
...
func UseNewInline() int {
	return new(AA)[0].Execute() + new(BB)[0].Run()
}
...
func UseFuncReturnInline() int {
	return getAS()[0].Execute() + getBS()[0].Run()
}
