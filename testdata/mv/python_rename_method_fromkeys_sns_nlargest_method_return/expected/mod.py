...
def use_fromkeys_sub(ba: BoxA, bb: BoxB) -> int:
    return (
        dict.fromkeys(["k"], ba.get())["k"].execute()
...
def use_fromkeys_values(ba: BoxA, bb: BoxB) -> int:
    return (
        list(dict.fromkeys(["k"], ba.get()).values())[0].execute()
...
def use_fromkeys_assign(ba: BoxA, bb: BoxB) -> int:
    ...
    return next(iter(da.values())).execute() + next(iter(db.values())).run()
...
def use_sns_inline(ba: BoxA, bb: BoxB) -> int:
    return (
        SimpleNamespace(k=ba.get()).k.execute()
