...
def use_mpt_sub(ba: BoxA, bb: BoxB) -> int:
    return (
        MappingProxyType({"k": ba.get()})["k"].run()
...
def use_mpt_get(ba: BoxA, bb: BoxB) -> int:
    return (
        MappingProxyType({"k": ba.get()}).get("k").run()
...
def use_mpt_values(ba: BoxA, bb: BoxB) -> int:
    return (
        list(MappingProxyType({"k": ba.get()}).values())[0].run()
...
def use_getitem_assign(ba: BoxA, bb: BoxB) -> int:
    ...
    return opa.run() + opb.run() + dua.run() + dub.run()


def use_dunder_direct(ba: BoxA, bb: BoxB) -> int:
    return (
        [ba.get()].__getitem__(0).run()
