...
async function useCatchThenIdentity() {
  return (
    (await Promise.resolve(new A()).catch(() => null).then((x) => x)).execute() +
...
async function useCatchFinally() {
  return (
    (await Promise.resolve(new A()).catch(() => null).finally(() => {})).execute() +
...
async function useIdent() {
  ...
  const b0 = new B();
  return (
    (await Promise.resolve(a0).catch(() => null)).execute() +
