...
class Uses {
  ...
  public static int useSupplyExecJoin(Executor ex) {
    return CompletableFuture.supplyAsync(() -> new A(), ex).join().execute()
  ...
  public static int useSupplyVarGetNow() {
    ...
    return fa2.getNow(null).execute() + fb2.getNow(null).run();
  }

  // Identity thenApply after supplyAsync factory + join.
  public static int useSupplyThenApplyJoin() {
    return CompletableFuture.supplyAsync(() -> new A()).thenApply(x1 -> x1).join().execute()
