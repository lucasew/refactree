...
class Uses {
  ...
  public static int useUnmodifiable(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.collectingAndThen(Collectors.toList(), Collections::unmodifiableList)).forEach(a -> a.execute());
  ...
  public static int useCollectingAndThenToSet(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.collectingAndThen(Collectors.toSet(), xs -> xs)).forEach(a -> a.execute());
