...
def use_deque_popleft(ba: BoxA, bb: BoxB) -> int:
    return deque([ba.get()]).popleft().execute() + deque([bb.get()]).popleft().run()


def use_deque_pop(ba: BoxA, bb: BoxB) -> int:
    return deque([ba.get()]).pop().execute() + deque([bb.get()]).pop().run()


def use_list_pop(ba: BoxA, bb: BoxB) -> int:
    return [ba.get()].pop().execute() + [bb.get()].pop().run()


def use_list_copy(ba: BoxA, bb: BoxB) -> int:
    return [ba.get()].copy()[0].execute() + [bb.get()].copy()[0].run()
...
def use_userlist(ba: BoxA, bb: BoxB) -> int:
    return UserList([ba.get()])[0].execute() + UserList([bb.get()])[0].run()
...
def use_dict_values_list_dict(ba: BoxA, bb: BoxB) -> int:
    return (
        list(dict(k=ba.get()).values())[0].execute()
