...
class Use {
  int useCollectToList(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).collect(Collectors.toList()).get(0).run()
      + Stream.of(bb.get()).collect(Collectors.toList()).get(0).run();
  }

  int useUnmodifiable(BoxA ba, BoxB bb) {
    return Collections.unmodifiableList(List.of(ba.get())).get(0).run()
  ...
  int useUnmodifiableAssign(BoxA ba, BoxB bb) {
    ...
    return xs.get(0).run() + ys.get(0).run();
  }
  ...
  int useClass() {
    return Stream.of(new A()).collect(Collectors.toList()).get(0).run()
      + Stream.of(new B()).collect(Collectors.toList()).get(0).run()
      + Collections.unmodifiableList(List.of(new A())).get(0).run()
