...
async function useResolveInline() {
  return (
    (await Promise.resolve(new BoxA().get())).run() +
...
function useResolveThen() {
  return (
    Promise.resolve(new BoxA().get()).then((a) => a.run()) +
...
async function useAllIndex() {
  return (
    (await Promise.all([new BoxA().get()]))[0].run() +
...
async function useRaceAny() {
  return (
    (await Promise.race([new BoxA().get()])).run() +
    (await Promise.race([new BoxB().get()])).run() +
    (await Promise.any([new BoxA().get()])).run() +
...
async function useSettledInline() {
  return (
    (await Promise.allSettled([new BoxA().get()]))[0].value.run() +
