...
class Use {
  ...
  int useMapConcurrent(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).gather(Gatherers.mapConcurrent(1, a -> a)).findFirst().get().run()
  ...
  int useClass() {
    return Stream.of(new A()).gather(Gatherers.mapConcurrent(1, a -> a)).findFirst().get().run()
