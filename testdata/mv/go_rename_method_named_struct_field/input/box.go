...
func UseNamed(wa WA, wb WB) int {
	return wa.Inner.Items[0].Run() + wb.Inner.Items[0].Run()
}

func UseNamedMap(wa WA, wb WB) int {
	return wa.Inner.M["k"].Run() + wb.Inner.M["k"].Run()
}
...
func UseNamedPtrField(wa WA, wb WB) int {
	return wa.InnerP.Items[0].Run() + wb.InnerP.Items[0].Run()
}

func UseNamedFuncField(wa WA, wb WB) int {
	return wa.Inner.Fa()[0].Run() + wb.Inner.Fa()[0].Run()
}

func UseMultiHop(xa XA, xb XB) int {
	return xa.Mid.Inner.Items[0].Run() + xb.Mid.Inner.Items[0].Run()
}
