...
class Use {
  ...
  int useOfEntries(BoxA ba, BoxB bb) {
    return Map.ofEntries(Map.entry("k", ba.get())).get("k").run()
  ...
  int useSimpleEntry(BoxA ba, BoxB bb) {
    return new AbstractMap.SimpleEntry<>("k", ba.get()).getValue().run()
        + new AbstractMap.SimpleEntry<>("k", bb.get()).getValue().run();
  }

  int useSimpleImm(BoxA ba, BoxB bb) {
    return new AbstractMap.SimpleImmutableEntry<>("k", ba.get()).getValue().run()
  ...
  int useSingletonMap(BoxA ba, BoxB bb) {
    return Collections.singletonMap("k", ba.get()).get("k").run()
  ...
  int useClass() {
    return Map.ofEntries(Map.entry("k", new A())).get("k").run()
        + Map.ofEntries(Map.entry("k", new B())).get("k").run()
        + new AbstractMap.SimpleEntry<>("k", new A()).getValue().run()
        + new AbstractMap.SimpleEntry<>("k", new B()).getValue().run()
        + Collections.singletonMap("k", new A()).get("k").run()
