...
class Uses {
  public static int useValuesForEach(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.collectingAndThen(Collectors.toMap(a -> "k", a -> a), m -> m)).values().forEach(a -> a.run());
  ...
  public static int useMapForEach(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.collectingAndThen(Collectors.toMap(a -> "k", a -> a), m -> m)).forEach((k, a) -> a.run());
  ...
  public static int useUnmodifiable(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.collectingAndThen(Collectors.toMap(a -> "k", a -> a), Collections::unmodifiableMap)).values().forEach(a -> a.run());
  ...
  public static int useConcurrent(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.collectingAndThen(Collectors.toConcurrentMap(a -> "k", a -> a), m -> m)).values().forEach(a -> a.run());
  ...
  public static int useMerge(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.collectingAndThen(Collectors.toMap(a -> "k", a -> a, (x, y) -> x), m -> m)).values().forEach(a -> a.run());
