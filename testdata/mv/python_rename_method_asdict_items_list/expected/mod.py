...
def use_list(box: Box) -> int:
    return list(asdict(box).items())[0][1].execute()


def use_tuple(box: Box) -> int:
    return tuple(asdict(box).items())[0][1].execute()


def use_dc_list(box: Box) -> int:
    return list(dataclasses.asdict(box).items())[0][1].execute()


def use_vars_list(box: Box) -> int:
    return list(vars(box).items())[0][1].execute()


def use_dunder_list(box: Box) -> int:
    return list(box.__dict__.items())[0][1].execute()


def use_assigned(box: Box) -> int:
    d = asdict(box)
    return list(d.items())[0][1].execute()
