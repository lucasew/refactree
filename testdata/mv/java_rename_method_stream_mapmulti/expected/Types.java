...
class Uses {
  ...
  public static int useMapMultiBody(List<A> as, List<B> bs) {
    as.stream().mapMulti((a, c) -> c.accept(a.execute())).forEach(i -> {});
  ...
  public static int useMapMultiForEach(List<A> as, List<B> bs) {
    as.stream().mapMulti((a, c) -> c.accept(a)).forEach(x -> x.execute());
  ...
  public static int useMapMultiNew(List<A> as, List<B> bs) {
    as.stream().mapMulti((a, c) -> c.accept(new A())).forEach(x -> x.execute());
