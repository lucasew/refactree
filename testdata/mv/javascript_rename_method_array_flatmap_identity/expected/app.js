...
function useFlatMapBare() {
  return (
    [new A()].flatMap(x => [x])[0].execute() + [new B()].flatMap(x => [x])[0].run()
  );
}
...
function useFlatMapAt() {
  return (
    [new A()].flatMap((x) => [x]).at(0).execute() +
...
function useFlatMapSlice() {
  return (
    [new A()].slice().flatMap((x) => [x])[0].execute() +
...
function useArrayFromFlatMap() {
  return (
    Array.from([new A()]).flatMap((x) => [x])[0].execute() +
...
function useIdent() {
  const a = new A();
  const b = new B();
  return [a].flatMap((x) => [x])[0].execute() + [b].flatMap((x) => [x])[0].run();
}
