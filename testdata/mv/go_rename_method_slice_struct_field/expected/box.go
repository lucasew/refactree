...
func UseSlice(sas []SA, sbs []SB) int {
	return sas[0].Items[0].Execute() + sbs[0].Items[0].Run()
}
...
func UseSliceMap(sas []SA, sbs []SB) int {
	return sas[0].M["k"].Execute() + sbs[0].M["k"].Run()
}

func UseMapOfStruct(mas map[string]SA, mbs map[string]SB) int {
	return mas["k"].Items[0].Execute() + mbs["k"].Items[0].Run()
}
...
func UseSliceFuncField(sas []SA, sbs []SB) int {
	return sas[0].Fa()[0].Execute() + sbs[0].Fa()[0].Run()
}
...
func UseSliceNested(sas []SA2, sbs []SB2) int {
	return sas[0].Inner.Items[0].Execute() + sbs[0].Inner.Items[0].Run()
}
