...
function useThenValue() {
  return (
    Promise.allSettled([new A()]).then(([ra]) => ra.value.execute()) +
...
function useThenObjDestr() {
  return (
    Promise.allSettled([new A()]).then(([{ value: a }]) => a.execute()) +
...
function useThenObjShorthand() {
  return (
    Promise.allSettled([new A()]).then(([{ value: va }]) => va.execute()) +
...
function useIdent() {
  ...
  const b = new B();
  return (
    Promise.allSettled([a]).then(([ra]) => ra.value.execute()) +
