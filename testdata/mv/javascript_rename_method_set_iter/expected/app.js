...
function useValuesNextValue() {
  return (
    new Set([new A()]).values().next().value.execute() +
...
function useKeysNextValue() {
  return (
    new Set([new A()]).keys().next().value.execute() +
...
function useSymbolIterator() {
  return (
    new Set([new A()])[Symbol.iterator]().next().value.execute() +
...
function useSetLocalValues() {
  ...
  return sa.values().next().value.execute() + sb.values().next().value.run();
}

function useSetLocalKeys() {
  const sa = new Set([new A()]);
  const sb = new Set([new B()]);
  return sa.keys().next().value.execute() + sb.keys().next().value.run();
}
...
function useArrayLocal() {
  ...
  const bs = [new B()];
  return (
    new Set(as).values().next().value.execute() +
...
function useIdent() {
  ...
  const b = new B();
  return (
    new Set([a]).values().next().value.execute() +
