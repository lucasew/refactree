...
function useReduceAcc() {
  return (
    [new A()].reduce((a, b) => a).execute() + [new B()].reduce((a, b) => a).run()
  );
}

function useReduceCur() {
  return (
    [new A()].reduce((a, b) => b).execute() + [new B()].reduce((a, b) => b).run()
  );
}
...
function useReduceInit() {
  return (
    [new A()].reduce((a, b) => a, new A()).execute() +
...
function useReduceArrayLocal() {
  const as = [new A()];
  const bs = [new B()];
  return as.reduce((a, b) => a).execute() + bs.reduce((a, b) => a).run();
}

function useReduceSlice() {
  return (
    [new A()].slice().reduce((a, b) => a).execute() +
...
function useArrayFromReduce() {
  return (
    Array.from([new A()]).reduce((a, b) => a).execute() +
...
function useIdent() {
  const a0 = new A();
  const b0 = new B();
  return [a0].reduce((a, b) => a).execute() + [b0].reduce((a, b) => a).run();
}


function useReduceRight() {
  return (
    [new A()].reduceRight((a, b) => a).execute() +
...
function useReduceRightCur() {
  return (
    [new A()].reduceRight((a, b) => b).execute() +
