...
class Use {
  ...
  int useStreamMap(BoxA ba, BoxB bb) {
    return Stream.of(0).map(x -> ba.get()).findFirst().get().execute()
        + Stream.of(0).map(x -> bb.get()).findFirst().get().run();
  }

  int useOptionalMap(BoxA ba, BoxB bb) {
    return Optional.of(0).map(x -> ba.get()).get().execute()
  ...
  int useStreamFlatMap(BoxA ba, BoxB bb) {
    return Stream.of(0).flatMap(x -> Stream.of(ba.get())).findFirst().get().execute()
        + Stream.of(0).flatMap(x -> Stream.of(bb.get())).findFirst().get().run();
  }

  int useOptionalFlatMap(BoxA ba, BoxB bb) {
    return Optional.of(0).flatMap(x -> Optional.of(ba.get())).get().execute()
  ...
  int useApplyToEither(BoxA ba, BoxB bb, CompletableFuture<Integer> o) throws Exception {
    return CompletableFuture.completedFuture(0).applyToEither(o, x -> ba.get()).join().execute()
  ...
  int useThenCombine(BoxA ba, BoxB bb, CompletableFuture<Integer> o) throws Exception {
    return CompletableFuture.completedFuture(0).thenCombine(o, (a, b) -> ba.get()).join().execute()
  ...
  int useApplyToEitherId(BoxA ba, BoxB bb, CompletableFuture<A> oa, CompletableFuture<B> ob)
      throws Exception {
    return CompletableFuture.completedFuture(ba.get()).applyToEither(oa, x -> x).join().execute()
  ...
  int useThenCombineId(BoxA ba, BoxB bb, CompletableFuture<A> oa, CompletableFuture<B> ob)
      throws Exception {
    return CompletableFuture.completedFuture(ba.get()).thenCombine(oa, (a, b) -> a).join().execute()
  ...
  int useVar(BoxA ba, BoxB bb) throws Exception {
    ...
    return fa.join().execute() + fb.join().run() + xa.execute() + xb.run();
  }

  // Class regression — already worked.
  int useClass(CompletableFuture<Integer> o) throws Exception {
    return Stream.of(0).map(x -> new A()).findFirst().get().execute()
        + Stream.of(0).map(x -> new B()).findFirst().get().run()
        + Optional.of(0).map(x -> new A()).get().execute()
        + Optional.of(0).map(x -> new B()).get().run()
        + CompletableFuture.completedFuture(0).thenApply(x -> new A()).join().execute()
        + CompletableFuture.completedFuture(0).thenApply(x -> new B()).join().run()
        + CompletableFuture.completedFuture(0).handle((x, e) -> new A()).join().execute()
        + CompletableFuture.completedFuture(0).handle((x, e) -> new B()).join().run()
        + Stream.of(0).flatMap(x -> Stream.of(new A())).findFirst().get().execute()
        + Stream.of(0).flatMap(x -> Stream.of(new B())).findFirst().get().run()
        + Optional.of(0).flatMap(x -> Optional.of(new A())).get().execute()
        ...
        + CompletableFuture.completedFuture(0)
            ...
            .join()
            .run()
        + CompletableFuture.completedFuture(0).applyToEither(o, x -> new A()).join().execute()
        + CompletableFuture.completedFuture(0).applyToEither(o, x -> new B()).join().run()
        + CompletableFuture.completedFuture(0).thenCombine(o, (a, b) -> new A()).join().execute()
