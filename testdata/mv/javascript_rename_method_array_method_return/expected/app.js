...
function useArrayOf() {
  return Array.of(new BoxA().get())[0].execute() + Array.of(new BoxB().get())[0].run();
}

function useAt() {
  return [new BoxA().get()].at(0).execute() + [new BoxB().get()].at(0).run();
}

function usePop() {
  return [new BoxA().get()].pop().execute() + [new BoxB().get()].pop().run();
}

function useValues() {
  return (
    Object.values({ k: new BoxA().get() })[0].execute() +
...
function useMapGet() {
  return (
    new Map([["k", new BoxA().get()]]).get("k").execute() +
...
function useSetSpread() {
  return (
    [...new Set([new BoxA().get()])][0].execute() +
...
function useNullish() {
  return (new BoxA().get() ?? null).execute() + (new BoxB().get() ?? null).run();
}
