...
class Use {
  int useFilter(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).filter(x -> true).findFirst().get().run()
  ...
  int useOr(BoxA ba, BoxB bb) {
    return Optional.of(ba.get()).or(() -> Optional.empty()).get().run()
