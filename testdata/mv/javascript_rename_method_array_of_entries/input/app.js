...
function useArrayOfIndex() {
  return Array.of(new A())[0].run() + Array.of(new B())[0].run();
}

function useArrayOfMulti() {
  return (
    Array.of(new A(), new A())[1].run() + Array.of(new B(), new B())[1].run()
  );
}
...
function useShorthand() {
  ...
  const b = new B();
  return (
    Object.entries({ a })[0][1].run() + Object.entries({ b })[0][1].run()
  );
}
