...
function useWith() {
  return (
    [new A()].with(0, new A())[0].execute() + [new B()].with(0, new B())[0].run()
  );
}
...
function useWithIdentVal() {
  const a = new A();
  const b = new B();
  return [new A()].with(0, a)[0].execute() + [new B()].with(0, b)[0].run();
}
...
function useWithAt() {
  return (
    [new A()].with(0, new A()).at(0).execute() +
...
function useWithNegIndex() {
  return (
    [new A()].with(-1, new A())[0].execute() + [new B()].with(-1, new B())[0].run()
  );
}

function useArrayFromWith() {
  return (
    Array.from([new A()]).with(0, new A())[0].execute() +
