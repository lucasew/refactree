...
function useFromEntriesMapInline() {
  return (
    Object.fromEntries(Map.groupBy([new A()], (x) => "k"))["k"][0].execute() +
...
function useFromEntriesMapLocal() {
  ...
  return oa["k"][0].execute() + ob["k"][0].run();
}

function useFromEntriesMapDot() {
  const ma = Map.groupBy([new A()], (x) => "k");
  const mb = Map.groupBy([new B()], (x) => "k");
  return Object.fromEntries(ma).k[0].execute() + Object.fromEntries(mb).k[0].run();
}

function useObjectValuesLocal() {
  const ga = Object.groupBy([new A()], (x) => "k");
  const gb = Object.groupBy([new B()], (x) => "k");
  return Object.values(ga)[0][0].execute() + Object.values(gb)[0][0].run();
}
...
function useMapValuesNext() {
  const ma = Map.groupBy([new A()], (x) => "k");
  const mb = Map.groupBy([new B()], (x) => "k");
  return ma.values().next().value[0].execute() + mb.values().next().value[0].run();
}

function useMapValuesNextInline() {
  return (
    Map.groupBy([new A()], (x) => "k").values().next().value[0].execute() +
