...
def use_mr_static(ba: BoxA, bb: BoxB) -> int:
    return A.from_box(ba).execute() + B.from_box(bb).run()


def use_mr_classmethod(ba: BoxA, bb: BoxB) -> int:
    return A.from_box_c(ba).execute() + B.from_box_c(bb).run()
...
def use_mr_static_body_assign(ba: BoxA, bb: BoxB) -> int:
    return A.from_box_assign(ba).execute() + B.from_box_assign(bb).run()
...
def use_nested_gen(ba: BoxA, bb: BoxB) -> int:
    ...
    return (
        next(gen_cna()).execute()
        + next(gen_cnb()).run()
        + next(gen_mna()).execute()
        + next(gen_mnb()).run()
        + next(gen_maa()).execute()
        + next(gen_mab()).run()
    )
...
def use_param_gen(ba: BoxA, bb: BoxB) -> int:
    return next(gen_mpa(ba)).execute() + next(gen_mpb(bb)).run()
