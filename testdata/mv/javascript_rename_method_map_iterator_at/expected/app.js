...
function useSymbolIteratorNext() {
  ...
  mb.set("k", new B());
  return (
    ma[Symbol.iterator]().next().value[1].execute() +
...
function useSymbolIteratorInline() {
  return (
    new Map([["k", new A()]])[Symbol.iterator]().next().value[1].execute() +
...
function useBareSpreadAt() {
  ...
  ma.set("k", new A());
  mb.set("k", new B());
  return [...ma].at(0)[1].execute() + [...mb].at(0)[1].run();
}

function useBareArrayFromAt() {
  const ma = new Map([["k", new A()]]);
  const mb = new Map([["k", new B()]]);
  return Array.from(ma).at(0)[1].execute() + Array.from(mb).at(0)[1].run();
}
...
function useEntriesSpreadAt() {
  ...
  const mb = new Map([["k", new B()]]);
  return (
    [...ma.entries()].at(0)[1].execute() + [...mb.entries()].at(0)[1].run()
  );
}
