...
def use_obj(ba: BoxA, bb: BoxB) -> int:
    return copy.copy(ba.get()).execute() + copy.deepcopy(bb.get()).run()


def use_list(ba: BoxA, bb: BoxB) -> int:
    return copy.copy([ba.get()])[0].execute() + copy.copy([bb.get()])[0].run()
...
def use_class() -> int:
    return copy.copy(A()).execute() + copy.copy(B()).run()
