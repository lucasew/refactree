...
def use_class_replace_inline() -> int:
    return NTA(A())._replace(x=A()).x.execute() + NTB(B())._replace(y=B()).y.run()


def use_class_replace_local() -> int:
    na = NTA(A())
    nb = NTB(B())
    return na._replace(x=A()).x.execute() + nb._replace(y=B()).y.run()
...
def use_class_replace_assign() -> int:
    ...
    ra = na._replace(x=A())
    rb = nb._replace(y=B())
    return ra.x.execute() + rb.y.run()


def use_class_make_inline() -> int:
    return NTA._make([A()]).x.execute() + NTB._make([B()]).y.run()


def use_class_make_index() -> int:
    return NTA._make([A()])[0].execute() + NTB._make([B()])[0].run()


def use_class_make_assign() -> int:
    na = NTA._make([A()])
    nb = NTB._make([B()])
    return na.x.execute() + nb.y.run()
...
def use_mr_replace_inline(ba: BoxA, bb: BoxB) -> int:
    return (
        NTA(ba.get())._replace(x=ba.get()).x.execute()
...
def use_mr_replace_local(ba: BoxA, bb: BoxB) -> int:
    na = NTA(A())
    nb = NTB(B())
    return na._replace(x=ba.get()).x.execute() + nb._replace(y=bb.get()).y.run()
...
def use_mr_make_inline(ba: BoxA, bb: BoxB) -> int:
    return NTA._make([ba.get()]).x.execute() + NTB._make([bb.get()]).y.run()


def use_mr_make_index(ba: BoxA, bb: BoxB) -> int:
    return NTA._make([ba.get()])[0].execute() + NTB._make([bb.get()])[0].run()
...
def use_mr_plain(ba: BoxA, bb: BoxB) -> int:
    return NTA(ba.get()).x.execute() + NTB(bb.get()).y.run()


def use_class_plain() -> int:
    return NTA(A()).x.execute() + NTB(B()).y.run()
