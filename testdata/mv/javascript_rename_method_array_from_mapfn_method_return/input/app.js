...
function useFromMapfnCtor() {
  return (
    Array.from([0], () => new BoxA().get())[0].run() +
...
function useFromMapfnCtorAt() {
  return (
    Array.from([0], (x) => new BoxA().get()).at(0).run() +
...
function useFromMapfnLength() {
  return (
    Array.from({ length: 1 }, () => new BoxA().get())[0].run() +
...
function useClassFromMapfn() {
  return (
    Array.from([0], () => new A())[0].run() +
    Array.from([0], () => new B())[0].run() +
    Array.from({ length: 1 }, () => new A())[0].run() +
