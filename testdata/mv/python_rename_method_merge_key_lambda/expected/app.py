...
def use_merge_key(xs: list[A], ys: list[A]):
    merge(xs, ys, key=lambda x: x.execute())


def use_merge_key_mod(xs: list[A], ys: list[A]):
    heapq.merge(xs, ys, key=lambda p: p.execute())
...
def use_merge_key_for(xs: list[A], ys: list[A]):
    for a in merge(xs, ys, key=lambda x: x.execute()):
        a.execute()
