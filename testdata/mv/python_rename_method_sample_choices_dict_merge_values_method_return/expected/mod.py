...
def use_class_sample_inline() -> int:
    return random.sample([A()], 1)[0].execute() + random.sample([B()], 1)[0].run()


def use_class_sample_bare() -> int:
    return sample([A()], 1)[0].execute() + sample([B()], 1)[0].run()


def use_class_choices_inline() -> int:
    return random.choices([A()], k=1)[0].execute() + random.choices([B()], k=1)[0].run()


def use_class_choices_bare() -> int:
    return choices([A()], k=1)[0].execute() + choices([B()], k=1)[0].run()
...
def use_mr_sample_inline(ba: BoxA, bb: BoxB) -> int:
    return random.sample([ba.get()], 1)[0].execute() + random.sample([bb.get()], 1)[0].run()


def use_mr_sample_bare(ba: BoxA, bb: BoxB) -> int:
    return sample([ba.get()], 1)[0].execute() + sample([bb.get()], 1)[0].run()


def use_mr_choices_inline(ba: BoxA, bb: BoxB) -> int:
    return random.choices([ba.get()], k=1)[0].execute() + random.choices([bb.get()], k=1)[0].run()


def use_mr_choices_bare(ba: BoxA, bb: BoxB) -> int:
    return choices([ba.get()], k=1)[0].execute() + choices([bb.get()], k=1)[0].run()
...
def use_mr_merge_values(ba: BoxA, bb: BoxB) -> int:
    return (
        list(({"k": ba.get()} | {}).values())[0].execute()
...
def use_mr_merge_values_multi(ba: BoxA, bb: BoxB) -> int:
    return (
        list(({"k": ba.get()} | {"j": ba.get()}).values())[0].execute()
...
def use_mr_choice(ba: BoxA, bb: BoxB) -> int:
    return random.choice([ba.get()]).execute() + random.choice([bb.get()]).run()


def use_class_choice() -> int:
    return random.choice([A()]).execute() + random.choice([B()]).run()
