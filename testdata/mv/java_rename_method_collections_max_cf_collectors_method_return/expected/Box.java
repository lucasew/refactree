...
class Use {
  int useCollectionsMax(BoxA ba, BoxB bb) {
    return Collections.max(List.of(ba.get()), Comparator.comparingInt(x -> 0)).execute()
        + Collections.max(List.of(bb.get()), Comparator.comparingInt(x -> 0)).run();
  }

  int useCollectionsMin(BoxA ba, BoxB bb) {
    return Collections.min(List.of(ba.get()), Comparator.comparingInt(x -> 0)).execute()
        + Collections.min(List.of(bb.get()), Comparator.comparingInt(x -> 0)).run();
  }

  int useCollectionsMaxClass() {
    return Collections.max(List.of(new A()), Comparator.comparingInt(x -> 0)).execute()
  ...
  int useHandle(BoxA ba, BoxB bb) {
    return CompletableFuture.completedFuture(ba.get()).handle((x, e) -> x).join().execute()
        + CompletableFuture.completedFuture(bb.get()).handle((x, e) -> x).join().run();
  }

  int useWhenComplete(BoxA ba, BoxB bb) {
    return CompletableFuture.completedFuture(ba.get()).whenComplete((x, e) -> {}).join().execute()
        + CompletableFuture.completedFuture(bb.get()).whenComplete((x, e) -> {}).join().run();
  }

  int useExceptionally(BoxA ba, BoxB bb) {
    return CompletableFuture.completedFuture(ba.get()).exceptionally(e -> ba.get()).join().execute()
        + CompletableFuture.completedFuture(bb.get()).exceptionally(e -> bb.get()).join().run();
  }

  int useHandleClass() {
    return CompletableFuture.completedFuture(new A()).handle((x, e) -> x).join().execute()
  ...
  int useReducing(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).collect(Collectors.reducing((a, b) -> a)).get().execute()
