...
function useSlice() {
  return [new A()].slice()[0].execute() + [new B()].slice()[0].run();
}

function useSliceArgs() {
  return [new A()].slice(0)[0].execute() + [new B()].slice(0)[0].run();
}
...
function useSliceAt() {
  return [new A()].slice().at(0).execute() + [new B()].slice().at(0).run();
}
...
function useConcat() {
  return [new A()].concat()[0].execute() + [new B()].concat()[0].run();
}

function useConcatArg() {
  return (
    [new A()].concat([new A()])[0].execute() +
...
function useConcatElem() {
  return (
    [new A()].concat(new A())[0].execute() + [new B()].concat(new B())[0].run()
  );
}
...
function useToSpliced() {
  return (
    [new A()].toSpliced(0, 0)[0].execute() + [new B()].toSpliced(0, 0)[0].run()
  );
}

function useToSplicedInsert() {
  return (
    [new A()].toSpliced(0, 0, new A())[0].execute() +
    [new B()].toSpliced(0, 0, new B())[0].run()
  );
}

function useFlatIdentity() {
  return [new A()].flat()[0].execute() + [new B()].flat()[0].run();
}
...
function useFlatDepth() {
  return [[new A()]].flat(1)[0].execute() + [[new B()]].flat(1)[0].run();
}

function useFlatNestedLocal() {
  const ia = [new A()];
  const ib = [new B()];
  return [ia].flat()[0].execute() + [ib].flat()[0].run();
}
...
function useArrayFromSlice() {
  return (
    Array.from([new A()]).slice()[0].execute() +
...
function useIdent() {
  const a = new A();
  const b = new B();
  return [a].slice()[0].execute() + [b].slice()[0].run();
}
