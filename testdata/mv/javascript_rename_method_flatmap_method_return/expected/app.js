...
function useIteratorFlatMap() {
  return (
    [new BoxA().get()].values().flatMap((x) => [x]).next().value.execute() +
...
function useIteratorFlatMapToArray() {
  return (
    [new BoxA().get()].values().flatMap((x) => [x]).toArray()[0].execute() +
...
function useArrayFlatMapCtor() {
  return (
    [0].flatMap(() => [new BoxA().get()])[0].execute() +
...
function useArrayFlatMapCtorAt() {
  return (
    [0].flatMap((x) => [new BoxA().get()]).at(0).execute() +
...
function useClassIteratorFlatMap() {
  return (
    [new A()].values().flatMap((x) => [x]).next().value.execute() +
