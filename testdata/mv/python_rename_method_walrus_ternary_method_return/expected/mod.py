...
def use_walrus_expr(ba: BoxA, bb: BoxB) -> int:
    return (a := ba.get()).execute() + (b := bb.get()).run()


def use_walrus_chain(ba: BoxA, bb: BoxB) -> int:
    return (xa := ba.self()).get().execute() + (xb := bb.self()).get().run()
...
def use_ternary_nested(ba: BoxA, bb: BoxB, c: bool) -> int:
    return (ba.self().get() if c else ba.get()).execute() + (bb.self().get() if c else bb.get()).run()


def use_ternary_recv(ba: BoxA, bb: BoxB, c: bool) -> int:
    return (ba if c else ba).get().execute() + (bb if c else bb).get().run()
