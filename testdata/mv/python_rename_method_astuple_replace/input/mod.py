...
def use_chain(box: Box) -> int:
    return astuple(replace(box))[0].run() + astuple(replace(box))[1].run()


def use_dc_chain(box: Box) -> int:
    return dataclasses.astuple(dataclasses.replace(box))[0].run() + dataclasses.astuple(dataclasses.replace(box))[1].run()


def use_mixed(box: Box) -> int:
    return astuple(dataclasses.replace(box))[0].run() + dataclasses.astuple(replace(box))[1].run()
