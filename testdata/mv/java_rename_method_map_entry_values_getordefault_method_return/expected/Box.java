...
class Use {
  ...
  int useGetOrDefault(BoxA ba, BoxB bb) {
    return Map.of("k", ba.get()).getOrDefault("k", null).execute()
  ...
  int usePutReturn(BoxA ba, BoxB bb) {
    return new HashMap<>(Map.of("k", ba.get())).put("k", ba.get()).execute()
        + new HashMap<>(Map.of("k", bb.get())).put("k", bb.get()).run();
  }

  // Class regression — already worked.
  int useClass() {
    return Map.of("k", new A()).getOrDefault("k", null).execute()
        + Map.of("k", new B()).getOrDefault("k", null).run()
        + new TreeMap<>(Map.of("k", new A())).firstEntry().getValue().execute()
        + new TreeMap<>(Map.of("k", new B())).firstEntry().getValue().run()
        + Map.of("k", new A()).values().iterator().next().execute()
        + Map.of("k", new B()).values().iterator().next().run()
        + Map.of("k", new A()).entrySet().iterator().next().getValue().execute()
