...
func UseSliceInline(as []*A, bs []*B) int {
	return as[:1][0].Run() + bs[:1][0].Run()
}

func UseSliceForms(as []*A, bs []*B) int {
	return as[1:][0].Run() + bs[1:][0].Run() + as[:][0].Run() + bs[:][0].Run() + as[0:1][0].Run() + bs[0:1][0].Run()
}

func UseSliceShort(as []*A, bs []*B) int {
	sa := as[:1]
	sb := bs[:1]
	return sa[0].Run() + sb[0].Run()
}
...
func UseSliceOfComposite() int {
	return []*A{&A{}}[:1][0].Run() + []*B{&B{}}[:1][0].Run()
}
