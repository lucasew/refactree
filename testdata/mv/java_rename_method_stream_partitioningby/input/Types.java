...
class Uses {
  public static int useValuesForEach(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.partitioningBy(a -> true)).values().forEach(g -> g.forEach(a -> a.run()));
  ...
  public static int useMapForEach(List<A> as, List<B> bs) {
    as.stream().collect(Collectors.partitioningBy(a -> true)).forEach((k, g) -> g.forEach(a -> a.run()));
