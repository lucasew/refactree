...
def use_proxy_get():
    return (
        MappingProxyType(vars(SimpleNamespace(k=A())))["k"].execute()
...
def use_proxy_values():
    return (
        next(iter(MappingProxyType(vars(SimpleNamespace(k=A()))).values())).execute()
...
def use_proxy_dunder():
    return (
        MappingProxyType(SimpleNamespace(k=A()).__dict__)["k"].execute()
