...
class Use {
  int useListOf(BoxA ba, BoxB bb) {
    return List.of(ba.get()).get(0).execute() + List.of(bb.get()).get(0).run();
  }

  int useArraysAsList(BoxA ba, BoxB bb) {
    return Arrays.asList(ba.get()).get(0).execute() + Arrays.asList(bb.get()).get(0).run();
  }

  int useSingletonList(BoxA ba, BoxB bb) {
    return Collections.singletonList(ba.get()).get(0).execute()
        + Collections.singletonList(bb.get()).get(0).run();
  }

  int useNCopies(BoxA ba, BoxB bb) {
    return Collections.nCopies(1, ba.get()).get(0).execute()
  ...
  int useStreamOfToList(BoxA ba, BoxB bb) {
    return Stream.of(ba.get()).toList().get(0).execute()
        + Stream.of(bb.get()).toList().get(0).run();
  }

  int useMapOf(BoxA ba, BoxB bb) {
    return Map.of("k", ba.get()).get("k").execute() + Map.of("k", bb.get()).get("k").run();
  }

  int useCompletedFuture(BoxA ba, BoxB bb) {
    return CompletableFuture.completedFuture(ba.get()).join().execute()
        + CompletableFuture.completedFuture(bb.get()).join().run();
  }

  int useIdentity(BoxA ba, BoxB bb) {
    return Function.identity().apply(ba.get()).execute()
        + Function.identity().apply(bb.get()).run();
  }

  int useIteratorNext(BoxA ba, BoxB bb) {
    return List.of(ba.get()).iterator().next().execute()
