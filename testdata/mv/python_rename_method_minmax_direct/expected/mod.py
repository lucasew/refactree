...
def use_min_values_direct(pair: Pair) -> int:
    return min(asdict(pair).values(), key=lambda v: 0).execute()


def use_max_values_direct(pair: Pair) -> int:
    return max(asdict(pair).values(), key=lambda v: 0).execute()


def use_min_no_key_direct(pair: Pair) -> int:
    return min(asdict(pair).values()).execute()


def use_dc_direct(pair: Pair) -> int:
    return min(dataclasses.asdict(pair).values(), key=lambda v: 0).execute()


def use_vars_direct(pair: Pair) -> int:
    return min(vars(pair).values(), key=lambda v: 0).execute()


def use_dunder_direct(pair: Pair) -> int:
    return min(pair.__dict__.values(), key=lambda v: 0).execute()


def use_list_values_direct(pair: Pair) -> int:
    return min(list(asdict(pair).values()), key=lambda v: 0).execute()


def use_astuple_direct(pair: Pair) -> int:
    return min(astuple(pair), key=lambda v: 0).execute()


def use_assigned_values_direct(pair: Pair) -> int:
    d = asdict(pair)
    return min(d.values(), key=lambda v: 0).execute()


def use_min_items_direct(items: list[A]) -> int:
    return min(items).execute()


def use_max_items_direct(xs: list[A]) -> int:
    return max(xs, key=lambda x: 0).execute()


def use_choice_direct(ys: list[A]) -> int:
    return choice(ys).execute()


def use_random_choice_direct(zs: list[A]) -> int:
    return random.choice(zs).execute()
