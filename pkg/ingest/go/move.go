...
func (moveDriver) ExtraRenameEdits(rootDir string, result *ingest.Result, sourceRefs []string, oldLeaf, newLeaf string) []ingest.Edit {
	...
	for _, f := range result.Files {
		...
		// Non-identifier receivers: v.(T).M, xs[0].M, Make().M, (expr).M.
...
func findSelectorLeafEdits(file string, content []byte, oldLeaf, newLeaf string, allowedReceivers map[string]bool) []ingest.Edit {
	...
	for {
		...
		// Complex operands (type assert, index, call, composite) are handled by
		// findComplexOperandSelectorEdits.
		recvStart := dot - 1
		if recvStart < 0 || !ingest.IsIdentChar(text[recvStart]) {
			off = end
			continue
...
// identifier: v.(T).M, (v.(T)).M, xs[i].M, Make().M, T{}.M / &T{}.M.
//
			...
			if field != nil && ingest.NodeText(field, content) == oldLeaf && operand != nil {
				if !goOperandIsBareIdent(operand) {
					...
					if typ := goComplexOperandType(operand, content); typ != "" {
						...
						// Index/call/etc. without static type: only when unique.
						ok = true
					}
					if ok {
						edits = append(edits, ingest.Edit{
...
// goOperandIsBareIdent reports whether n is a plain identifier (optionally
// parenthesized), which findSelectorLeafEdits already covers via text scan.
func goOperandIsBareIdent(n *grammar.Node) bool {
	...
	case "parenthesized_expression":
		for i := uint32(0); i < n.ChildCount(); i++ {
			...
			return goOperandIsBareIdent(ch)
		}
	}
	return false
}
...
// full type inference: type assert/convert, composite lit, &T{}, parenthesized.
func goComplexOperandType(n *grammar.Node, content []byte) string {
	...
	case "unary_expression":
		// &T{}
		for i := uint32(0); i < n.ChildCount(); i++ {
			if t := goComplexOperandType(n.Child(i), content); t != "" {
				return t
			}
		}
	}
